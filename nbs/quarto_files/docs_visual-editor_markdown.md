...
5.  [Markdown Output](../../docs/visual-editor/markdown.html)
        ...
        -   <span class="sidebar-item-text sidebar-link text-start"
            ...
            -   <a href="../../docs/visual-editor/index.html"
                ...
                -   <a href="../../docs/visual-editor/markdown.html"
                    ...
                    class="menu-text">Markdown Output</span></a>
...
-   <a href="#writer-options" id="toc-writer-options" class="nav-link"
    ...
    -   <a href="#line-wrapping" id="toc-line-wrapping" class="nav-link"
        data-scroll-target="#line-wrapping">Line Wrapping</a>
    -   <a href="#references" id="toc-references" class="nav-link"
        data-scroll-target="#references">References</a>
    -   <a href="#canonical-mode" id="toc-canonical-mode" class="nav-link"
        data-scroll-target="#canonical-mode">Canonical Mode</a>
-   <a href="#known-limitations" id="toc-known-limitations" class="nav-link"
    data-scroll-target="#known-limitations">Known Limitations</a>

-   <a
    href="https://github.com/quarto-dev/quarto-web/edit/main/docs/visual-editor/markdown.qmd"
...
does. Note that you can also [configure source mode](#canonical-mode) to
write markdown using these conventions, ensuring that the same markdown
is written no matter which mode edits originate from.
...
-   Whether to use the visual mode markdown writer when saving markdown
    from source mode (to ensure consistency between documents saved from
    either mode).

You can set these options within the **R Markdown** [Global
Options](../../docs/visual-editor/options.html#global-options) or
[Project
Options](../../docs/visual-editor/options.html#project-options), or can
alternatively set them on a per-file basis using YAML (as described
below).

### Line Wrapping

By default, the visual editor writes Markdown with no line wrapping
(paragraphs all occupy a single line). This matches the behavior of
markdown source editing mode within RStudio.

However, if you prefer to insert line breaks at a particular column
(e.g. 72 or 80), or to insert a line break after each sentence, you can
set a global or per-project [editor
option](../../docs/visual-editor/options.html#global-options) to this
effect.

You can also set this behavior on a per-document basis via the `wrap`
option. For example, to wrap lines after 72 characters you would use
this:
...
By default, references are written at the end of the block where their
corresponding footnote appears. You can override this behavior using the
`references` option.
...
Note that you can also set a global or per-project [editor
option](../../docs/visual-editor/options.html#global-options) to control
reference writing behavior.
...
Note that if you are within a Quarto [book](../../docs/books/index.html)
project then a references `prefix` is applied automatically so no
changes to `editor: markdown` are required.

### Canonical Mode

If you have a workflow that involves editing in both visual and source
mode, you may want to ensure that the same markdown is written no matter
which mode edits originate from. You can accomplish this using the
`canonical` option. For example:
...
With `canonical: true`, edits in visual mode and source mode will result
in identical markdown output. This is especially useful if you have
multiple authors collaborating using version control, with a mixture of
source and visual mode editing among the authors.
...
<a href="../../docs/visual-editor/options.html"
class="pagination-link aria-label=" data-shortcuts="" data-&amp;=""
data-options"=""><em></em> <span class="nav-page-text">Shortcuts &amp;
Options</span></a>

<a href="../../docs/tools/vscode.html" class="pagination-link"
aria-label="VS Code"><span class="nav-page-text">VS Code</span>
<em></em></a>
