...
4.  [GitHub (GFM)](../../docs/output-formats/gfm.html)
    ...
    -   <span class="sidebar-item-text sidebar-link text-start"
        ...
        -   <span class="sidebar-item-text sidebar-link text-start"
            ...
            -   <a href="../../docs/output-formats/gfm.html"
                ...
                class="menu-text">GitHub (GFM)</span></a>
...
-   <a href="#gfm-format" id="toc-gfm-format" class="nav-link"
    data-scroll-target="#gfm-format">GFM Format</a>
-   <a href="#preview-mode" id="toc-preview-mode" class="nav-link"
    data-scroll-target="#preview-mode">Preview Mode</a>
-   <a href="#webtex-math" id="toc-webtex-math" class="nav-link"
    data-scroll-target="#webtex-math">WebTeX Math</a>
    -   <a href="#dark-mode" id="toc-dark-mode" class="nav-link"
        data-scroll-target="#dark-mode">Dark Mode</a>
-   <a href="#github-wikis" id="toc-github-wikis" class="nav-link"
    data-scroll-target="#github-wikis">GitHub Wikis</a>

-   <a
    href="https://github.com/quarto-dev/quarto-web/edit/main/docs/output-formats/gfm.qmd"
...
While markdown is the input format for Quarto, it can also in some cases
be an output format (for example, if you have a website or CMS that
accepts markdown as input and want to incorporate computations from
Python or R).

This article covers using Quarto to generate [GitHub Flavored
Markdown](https://github.github.com/gfm/) (GFM). You might want to do
this in order to:

-   Generate a GitHub README.md from a Jupyter notebook

-   Create pages for a GitHub wiki that include computations (e.g. plot
    output).

## GFM Format

Use the `gfm` format to create GitHub Flavored Markdown from Quarto. For
example:
...
See the GFM [format
reference](../../docs/reference/formats/markdown/gfm.html) for a
complete list of all options available for GFM output.

To create a `README.md` using Quarto, start with a notebook (.ipynb) or
computational markdown file (.qmd) that has README as its file name
stem, for example:
...
When you `quarto preview` a GitHub Flavored Markdown document, by
default an HTML preview that approximates the look of markdown rendered
on GitHub is shown. If you’d prefer to see the raw generated markdown,
use the `preview-mode: raw` option. For example:
...
The `gfm` format renders LaTeX equations using standard dollar-delimited
inline (`$...$`) and display (`$$...$$`) syntax. However, if the web
environment you are publishing into doesn’t support dollar-delimited
math, you can alternatively use
[WebTeX](https://github.com/KTHse/webtex) to display math. This is done
by setting the Pandoc `html-math-method` to `webtex`. For example:
...
overall appearance. However, if your `gfm` document is being rendered on
a dark background, you may want to switch to PNG with a dark background
specified. You can do this as follows:
...
If you want to use Quarto to incorporate computations into a GitHub wiki
start by [cloning the wiki for local
editing](https://docs.github.com/en/communities/documenting-your-project-with-wikis/adding-or-editing-wiki-pages#adding-or-editing-wiki-pages-locally).

Then, simply create a computational markdown file (.ipynb, .qmd) for
each page in the wiki. You can render all of these files at once into
their corresponding .md files using [Quarto
Projects](../../docs/projects/quarto-projects.html). For example:

    Terminal

    quarto render

You don’t even strictly need a Quarto project file to do this as
`quarto render` will render all input files in a directory by default if
there is no project file.

<a href="../../docs/output-formats/ms-word-templates.html"
class="pagination-link aria-label=" data-word=""
data-templates"=""><em></em> <span class="nav-page-text">Word
Templates</span></a>

<a href="../../docs/output-formats/hugo.html" class="pagination-link"
aria-label="Hugo"><span class="nav-page-text">Hugo</span> <em></em></a>
