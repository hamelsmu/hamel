...
3.  [Conditional Content](../../docs/authoring/conditional.html)
...
-   <a href="#content-visible" id="toc-content-visible"
    class="nav-link active"
    data-scroll-target="#content-visible">.content-visible</a>
-   <a href="#content-hidden" id="toc-content-hidden" class="nav-link"
    data-scroll-target="#content-hidden">.content-hidden</a>
-   <a href="#format-matching" id="toc-format-matching" class="nav-link"
    data-scroll-target="#format-matching">Format Matching</a>

-   <a
    href="https://github.com/quarto-dev/quarto-web/edit/main/docs/authoring/conditional.qmd"
...
In some cases you may want to create content that only displays for a
given output format (or only displays when *not* rendering to a format).
You can accomplish this by creating divs, spans and code blocks with the
`.content-visible` and `.content-hidden` classes.

## .content-visible

To make content visible only for a given format, you can create a div
(`:::`) with the `.content-visible` class. For example, here we mark
content as only visible in HTML:
...
You can also mark content as visible for all formats *except* a
...
Then `when-format` and `unless-format` attributes match the current
Pandoc output format with some additional intelligence to alias related
formats (e.g. html, html4, and html5). Details are provided below in
[Format Matching](#format-matching)

`when-format` and `unless-format` can also be combined to create more
complex conditions:
...
To prevent content from being displayed when rendering to a given
format, use the `.content-hidden` class. For example, here we mark
content as hidden in HTML:
...
You can also mark content as hidden for all formats *except* a specified
format. For example:
...
Then `when-format` and `unless-format` clauses do some aliasing of
related formats to make it more straightforward to target content. The
following aliases are implemented:
...
<a href="../../docs/authoring/language.html"
class="pagination-link aria-label=" data-document=""
data-language"=""><em></em> <span class="nav-page-text">Document
Language</span></a>

<a href="../../docs/extensions/nbfilter.html" class="pagination-link"
aria-label="Notebook Filters"><span class="nav-page-text">Notebook
Filters</span> <em></em></a>
