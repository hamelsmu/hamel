...
3.  [Overview](../../docs/interactive/index.html)
...
-   <a href="#observable-js" id="toc-observable-js" class="nav-link"
    data-scroll-target="#observable-js">Observable JS</a>
-   <a href="#shiny" id="toc-shiny" class="nav-link"
    data-scroll-target="#shiny">Shiny</a>
-   <a href="#widgets" id="toc-widgets" class="nav-link"
    data-scroll-target="#widgets">Widgets</a>
-   <a href="#layout" id="toc-layout" class="nav-link"
    data-scroll-target="#layout">Layout</a>
-   <a href="#observable-js-on-the-rstudio-ide"
    id="toc-observable-js-on-the-rstudio-ide" class="nav-link"
    data-scroll-target="#observable-js-on-the-rstudio-ide">Observable JS on
    the RStudio IDE</a>

-   <a
    href="https://github.com/quarto-dev/quarto-web/edit/main/docs/interactive/index.qmd"
...
Adding interactivity to an article is a great way to help readers
explore the concepts and data you are presenting more deeply. There are
three ways to add interactive components to Quarto documents:

1.  Create custom JavaScript visualizations using [Observable JS](ojs/).

2.  Use the [Shiny](shiny/) R package to add interactivity to Knitr
    engine documents.

3.  Incorporate [Jupyter
    Widgets](../../docs/interactive/widgets/jupyter.html) or
    [htmlwidgets](../../docs/interactive/widgets/htmlwidgets.html) (for
    the Jupyter and Knitr engines, respectively) into your document.

Each of these techniques has distinct benefits and drawbacks in terms of
expressiveness, ease of development, and deployment requirements. We’ll
touch on these considerations briefly below, then provide links to more
in depth documentation for learning more.
...
Here’s an example that provides slider inputs to condition the behavior
of a visualization:
...
Observable JS uses some special keywords and a custom runtime to make
JavaScript reactive. For example, the “minimum fame” slider in the
example above was created with the following code:
...
It’s then referenced as a normal JavaScript variable in code that
creates the plot:

    sel.filter(d => d.fame <= minimum)

As the user interacts with the slider, the `minimum` value is updated
and any code that references it is automatically re-executed.

One benefit of using JavaScript for interactive documents is that all
the logic and computation is performed on the client (so no server is
...
The Shiny package provides a flexible, easy to use framework for
creating interactive web applications with R. Quarto in turn includes
support for embedding Shiny components and applets into documents
created with the Knitr engine.

Here’s a live example of Shiny interactive components along with a brief
explanation of the code required to create them:
...
<a href="_w_893e6f82/#" class="slider-animate-button" data-target-id="f"
data-interval="100" data-loop="FALSE"><span class="play"> <em></em>
</span> <span class="pause"> <em></em> </span></a>

Higher values give more smoothness.

<a href="https://www.google.com/finance/domestic_trends"
target="_blank">Source: Google Domestic Trends</a>

  

Shiny comes with a variety of built in input widgets. With minimal
syntax it is possible to include widgets like the ones shown on the left
in your apps:
...
Build your plots or tables as you normally would in R, and make them
reactive with a call to the appropriate render function:
...
Shiny makes it very straightforward to create interactive documents
using only R. Unlike using JavaScript though, you will need to
[deploy](../../docs/interactive/shiny/running.html#deployment) documents
that use Shiny to a server.

To learn more see the articles on [Using Shiny with Quarto](shiny/).

## Widgets

[Jupyter Widgets](https://jupyter.org/widgets) and
[htmlwidgets](https://www.htmlwidgets.org/) are great ways to
incorporate interactivity into your documents if you don’t know
JavaScript and prefer to work exclusively in Python or R. They also run
entirely client-side so can be deployed within normal static HTML
documents.

For example, the following Python code is all that is required to embed
a Leaflet map into a Jupyter engine document:
...
-   [Jupyter Widgets](../../docs/interactive/widgets/jupyter.html)
    (Jupyter engine).

-   [htmlwidgets](../../docs/interactive/widgets/htmlwidgets.html)
    (Knitr engine).

## Layout

Once you’ve gotten familiar with using various interactive components
see the article on [Component
Layout](../../docs/interactive/layout.html) to learn how to:

-   Group inputs into an [input
    panel](../../docs/interactive/layout.html#input-panel).

-   Present multiple outputs in a [tabset
    panel](../../docs/interactive/layout.html#tabset-panel).

-   Use a [full page
    layout](../../docs/interactive/layout.html#full-page-layout) rather
    than the default article layout.

-   Add a [sidebar
    panel](../../docs/interactive/layout.html#sidebar-panel) for inputs
    in a full page layout.

-   Create custom [panel
    layouts](../../docs/interactive/layout.html#panel-layout) to arrange
    outputs into rows and columns.

## Observable JS on the RStudio IDE

Observable JS offers full access to [NPM](https://www.npmjs.com/)
libraries, and these tend to use JS features that require the Electron
version of the RStudio IDE [daily builds](https://dailies.rstudio.com/).
If you plan on using Observable JS in the RStudio IDE, we recommend the
Electron daily builds.

<a href="../../docs/books/book-output.html"
class="pagination-link aria-label=" data-customizing=""
data-output"=""><em></em> <span class="nav-page-text">Customizing
Output</span></a>

<a href="../../docs/interactive/ojs/index.html" class="pagination-link"
aria-label="Observable JS"><span class="nav-page-text">Observable
JS</span> <em></em></a>
